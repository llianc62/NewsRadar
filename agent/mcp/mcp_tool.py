...
class MCPTool(BaseTool):
    """MCP 工具——通过 MCPClient 代理执行。

    由 Registry.add_mcp() 自动创建。
    """

    def __init__(self, session: MCPClient, tool_info: dict, level: int = 1):
        if not isinstance(session, MCPClient):
            raise TypeError("session must be an MCPClient instance")
    ...
    async def execute(self, **kwargs: Any) -> str:
        try:
            return await self._session.call_tool(self._name, kwargs)
